    ...
    .relation-div {
  	...
  	<script type="text/javascript">
  		...
  		var setting = {
			...
			callback: {
   				onClick: function (event, treeId, treeNode, clickFlag) {
   					$('#departmentTable').datagrid('load', getQueryParams(treeNode.id));
   					$('#roleTable').datagrid('load', getQueryParams(treeNode.id));
   					$('#personTable').datagrid('load', getQueryParams(treeNode.id));
   				}
			}
		};
	  	
	  	function initResourceTree(nodeId) {
	  		$.ajax({  
                type: "POST",  
                url: "SiteSearch/getResourceTree",
                success: function(resourceList) {
                	$.fn.zTree.init($("#tree"), setting, resourceList);
		...
		$(document).ready(function(){
			initResourceTree(${resourceId });
			
			$('#departmentTable').datagrid({
				title:'部门列表',
				width: 640,
				height: 150,
				url: "SiteSearch/getDepartments", 
				queryParams: getQueryParams(${resourceId }), 
				columns:[[
				...
				onClickRow: function(rowIndex, rowData) {
		    		window.location.href = "SiteSearch/showDepartmentInfo?id=" + rowData.id;
			    }
			});
			...
			$('#personTable').datagrid({
				...
				columns:[[
					...
					{field:'resourceName',title:'所在部门',width:100},
